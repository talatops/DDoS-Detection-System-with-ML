...
bool OpenCLHost::findDevice(const std::string& device_name) {
    cl_uint num_devices;
    cl_int err = clGetDeviceIDs(platform_, CL_DEVICE_TYPE_GPU, 0, nullptr, &num_devices);
...
double OpenCLHost::getKernelTime(const std::string& kernel_name) {
    ...
    cl_ulong start, end;
    clGetEventProfilingInfo(kernel_events_[event_idx], CL_PROFILING_COMMAND_START,
                           sizeof(start), &start, nullptr);
    clGetEventProfilingInfo(kernel_events_[event_idx], CL_PROFILING_COMMAND_END,
                           sizeof(end), &end, nullptr);
    
    return (end - start) * 1e-6;  // Convert to milliseconds
}
